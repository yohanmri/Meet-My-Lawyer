...
const Lawyers = () => {
  ...
  const applyFilter = () => {
    if (speciality) {
      setFilterLawyer(lawyers.filter(lawyer => lawyer.speciality === speciality))
    } else {
  ...
  return (
    <div>
      <p className='text-gray-600'>Brows through the lawyers specialist.</p>
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>

        <button className={`py-1 px-3 border rounded text-sm transition-all sm:hidden ${showFilter ? 'bg-primary text-white' : ''}`} onClick={() => setShowFilter(prev => !prev)}>Filters</button>
        <div className={`flex-col gap-4 text-sm text-gray-600 ${showFilter ? 'flex' : 'hidden sm:flex'}`}>
          <p onClick={() => speciality === 'General physician' ? navigate('/lawyers') : navigate('/lawyers/General physician')} className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${speciality === "General physician" ? "bg-indigo-100 text-black" : ""}`}>General physician</p>
          ...
          <p onClick={() => speciality === 'Dermatologist' ? navigate('/lawyers') : navigate('/lawyers/Dermatologist')} className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${speciality === "Dermatologist" ? "bg-indigo-100 text-black" : ""}`}>Dermatologist</p>
        ...
        <div className='w-full grid grid-cols-auto gap-4 gap-y-6'>
          {
            filterLawyer.map((item, index) => (
              <div onClick={() => navigate(`/appointment/${item._id}`)} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-5' key={index}>
