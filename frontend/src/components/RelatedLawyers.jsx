    ...
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
      ...
      <div className='w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
        {relLawyer.slice(0, 5).map((item, index) => (
          <div
            ...
            className='bg-gradient-to-br from-black to-[#030303] rounded-2xl p-3 cursor-pointer hover:translate-y-[-5px] transition-all duration-500 shadow-lg hover:shadow-xl'
            key={index}
          >
      ...
      {relLawyer.length === 0 && (
        <div className='text-center py-12'>
          ...
          <h3 className='text-2xl font-bold text-gray-900 mb-3'>No related lawyers found</h3>
          <p className='text-gray-600 mb-6 text-lg'>
            No lawyers available in this specialty at the moment
          </p>
        </div>
      )}
      ...
      <button
        ...
        className='bg-blue-50 text-[#6A0610] px-12 py-3 rounded-full mt-10 hover:scale-105 transition-all'
      >
        Show More
      </button>
    </div>
